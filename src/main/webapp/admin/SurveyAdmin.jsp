...
<body>
	<div class=nav>
		<a href=admin_main.jsp>桌面</a>»问卷列表
		<hr>
	</div>
	<table class=table cellspacing="0" cellpadding="0" align="center">
		<tbody>
			...
			<tr>
				<td><%=survey.getSId()%></td>
				<td><%=survey.getSName().length()>12?survey.getSName().substring(0,11)+"...":survey.getSName()%></td>
				<td><%=survey.getSAuthor().length()>12?survey.getSName().substring(0,11)+"...":survey.getSName()%></td>
				...
				<td><a href="SurveyShow.jsp?sid=<%=survey.getSId()%>">预览</a>
					|<%=survey.getSIsAudited()?"<span title='为了保证数据完整和准确性，已经通过审核的问卷不能再被编辑！'><font color=grey>编辑问卷</font></span>|<span title='为了保证数据完整和准确性，已经通过审核的问卷不能再被编辑！'><font color=grey>管理题目</font></span>"
					:"<a href=SurveyEdit.jsp?sid="+survey.getSId()+ ">编辑问卷</a>|<a href=QuestionAdmin.jsp?sid="+survey.getSId()+">管理题目</a>"%>
			...
			%>
			<tr>
				<td colspan=7 align="right"><%=pc.getCurrentPageHTML()%><%=pc.getCountPageHTML()%>|<%=pc.getFirstPageHTML()%>|<%=pc.getPageUpHTML()%>|<%=pc.getPageDownHTML()%>|<%=pc.getLastPageHTML()%></td>
			</tr>
		</tbody>
	</table>
</body>
